...
class Vocabulary:
    ...
    def save_data(self, filename):
        ...
        np.save(filename + '_word2index', tmp)
        tmp = [(k, v) for k, v in self.index2word.items()]
        np.save(filename + '_index2word', tmp)

    def load_data(self, filename):
        self.word2index.clear()
        self.index2word.clear()
        tmp = np.load(filename + "_word2index.npy", allow_pickle=True)
        for (k, v) in tmp:
            self.word2index[k] = int(v)
        tmp = np.load(filename + "_index2word.npy", allow_pickle=True)
        for (k, v) in tmp:
